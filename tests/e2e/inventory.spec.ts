...
const routes = ['/', '/databarang.html', '/history.html', '/laporan.html', '/laporanbarangmasuk.html', '/laporanbarangkeluar.html', '/laporanstokbarang.html', '/laporanpendapatan.html', '/pendapatanharian.html', '/pendapatanmingguan.html', '/pendapatanbulanan.html', '/pengaturan.html', '/logsinput.html', '/profilsetting.html']
...
async function processVoiceCart(page: Page) {
...
test('Android native voice works with granted microphone permission and shade action opens voice', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    fakeWindow.Capacitor = {
      PluginHeaders: [
        ...
        { name: 'LocalNotifications', methods: methods(['addListener', 'removeListener', 'createChannel', 'registerActionTypes', 'checkPermissions', 'requestPermissions', 'areEnabled', 'listChannels', 'schedule', 'getDeliveredNotifications', 'cancel', 'removeDeliveredNotificationsById', 'checkExactNotificationSetting', 'changeExactNotificationSetting']) },
      ],
  ...
  expect(await page.evaluate(() => (window as typeof window & { nativeRequests: { plugin: string; method: string; options: { notifications?: { actionTypeId?: string; channelId?: string; isExactNotification?: boolean }[] } }[] }).nativeRequests.find(request => request.plugin === 'LocalNotifications' && request.method === 'schedule')?.options.notifications?.[0])).toMatchObject({ actionTypeId: 'inventory_actions', channelId: 'inventory_status_v2', isExactNotification: false })
  expect(await page.evaluate(() => (window as typeof window & { nativeCalls: string[] }).nativeCalls.some(call => /ExactNotificationSetting/.test(call)))).toBe(false)
...
test('Android notification denial is visible and can be retried from the bell', async ({ page }) => {
  ...
  await page.goto('/')
  const retry = page.getByRole('button', { name: 'Aktifkan notifikasi Android' })
...
test('edit and delete transactions update stock, revenue, and input logs', async ({ page }) => {
  ...
  await expect(page.getByRole('row').filter({ hasText: 'Hapus transaksi: Speaker Bluetooth Mini' })).toBeVisible()
  await expect(page.getByRole('row').filter({ hasText: 'Edit transaksi: Speaker Bluetooth Mini' }).first()).toBeVisible()
})
...
test('add, edit, barcode export, search, delete product', async ({ page }) => {
  ...
  const pngDownload = page.waitForEvent('download'); await page.getByRole('button',{name:'Save PNG',exact:true}).click()
...
test('voice correction changes pending product and quantity before confirmation', async ({page}) => {
  ...
  const before = await data(page)
  await expect(page.getByRole('button', { name: 'Konfirmasi', exact: true })).toBeEnabled()
  await page.getByLabel(/Hasil voice/).fill('bukan anker maksud saya jbl')
  ...
  await page.getByLabel(/Hasil voice/).fill('qty tiga')
  await page.getByRole('button', { name: 'Pahami perintah' }).click()
  const corrected = page.getByRole('region', { name: 'Konfirmasi transaksi suara' })
  await expect(corrected.getByText('Qty')).toBeVisible()
  await expect(corrected.getByText('3', { exact: true }).first()).toBeVisible()
  await processVoiceCart(page)
  const after = await data(page)
...
test('spoken cancel clears pending voice transaction without writing', async ({page}) => {
  ...
  const before = await data(page)
  await expect(page.getByRole('button', { name: 'Konfirmasi', exact: true })).toBeEnabled()
  await page.getByLabel(/Hasil voice/).fill('batal')
  await page.getByRole('button', { name: 'Pahami perintah' }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
...
test('microphone denied preserves dialog and supports text fallback',async ({page})=>{
  ...
  await page.getByLabel(/Hasil voice/).fill('restok charger Anker lima')
  await page.getByRole('button',{name:'Pahami perintah'}).click()
  await expect(page.getByRole('button',{name:'Konfirmasi',exact:true})).toBeEnabled()
})
...
test('negative stock, missing product and unknown intent never write',async ({page})=>{
  ...
  for (const command of ['jual barangtidakada dua','perintah yang tidak diketahui']) {
    await page.getByLabel(/Hasil voice/).fill(command); await page.getByRole('button',{name:'Pahami perintah'}).click()
  }
  expect(await data(page)).toEqual(before)
})
...
test('voice transaction detects comma and sequential items over two products', async ({page}) => {
  ...
  await expect(region.getByText('Qty').first()).toBeVisible()
  await expect(region.getByText('2', { exact: true }).first()).toBeVisible()
  await expect(region.getByText('1', { exact: true }).first()).toBeVisible()
  await expect(region.getByText('3', { exact: true }).first()).toBeVisible()
  await processVoiceCart(page)
  const after=await data(page)
...
for(const [error,message] of [['audio-capture','Microphone tidak tersedia'],['no-speech','Suara belum terdengar'],['network','Ketik perintah']]) {
  test(`voice error ${error} keeps transcript UI available`,async ({page})=>{
    ...
    await expect(page.getByLabel(/Hasil voice/)).toBeEnabled()
