...
test('login fits mobile and password visibility can be toggled', async ({ page }) => {
  ...
  await expect(openMascot).toHaveJSProperty('complete', true)
  await expect(coveredMascot).toHaveJSProperty('complete', true)
  await expect(coveredMascot).not.toHaveJSProperty('naturalWidth', 0)
